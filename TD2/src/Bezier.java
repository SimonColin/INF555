...
public class Bezier extends Curve {
	...
	public void subdivisionRendering(int n) {
		...
		for(int i = 0; i < n; i++)
		{
			LinkedList<Bezier> tmp = new LinkedList<Bezier>();
			for(Bezier b : subCurves)
			{
				Bezier[] res = b.subdivide(0.5);
				tmp.add(res[0]);
				tmp.add(res[1]);
			}
			subCurves = tmp;
	...
	public Point_2 bernsteinBezier(double dt)
	{
		...
		Point_2 pt;
		Vector_2 v;
		Point_2 out = this.points[0].multiplyByScalar(binco(n - 1, 0));
		for(int i = 1; i < n; i++)
		{
			pt = this.points[i].multiplyByScalar(binco(n - 1, i) * t);
			v = new Vector_2(pt.x, pt.y);
			out = out.multiplyByScalar(s);
			out = out.sum(v);
			t = t * dt;
		}
